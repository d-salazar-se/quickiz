...
class QuizEditComponent extends Component {
  ...
  getQuizStructure() {
    axios.get(process.env.REACT_APP_API_URL + '/quizstructures/' + this.state.quiz.id)
      .then((result) => {
        ...
        for (var i = quiz.questionsStructures.length - 1; i >= 0; i--) {
          quiz.questionsStructures[i] = {
            score: quiz.questionsStructures[i].score,
            poolId: quiz.questionsStructures[i].pool.id
          };
        }
  ...
  handleSubmit(event) {
    ...
    axios.put(process.env.REACT_APP_API_URL + '/quizstructures/' + this.state.quiz.id, {
      "quiz": quiz, "questions": questions
    })
     .then(result => {
        this.props.history.push('/quizzes/');
        this.props.enqueueSnackbar('Quiz actualizado!', { variant: 'success' });
      })
      .catch(error => {
                ...
                <Grid container direction="row" alignItems="center" spacing={24}>
                  ...
                  <Grid item xs={3}>
                    <TextField
                              ...
                              label="Holgura"
                              placeholder="Holgura"
                              value={this.state.quiz.slacktime}
...
export default withSnackbar(QuizEditComponent);
